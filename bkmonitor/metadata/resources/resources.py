...
from metadata.resources.time_series import (
    ...
    QueryTimeSeriesMetricRequestSerializer,
...
class QueryTimeSeriesMetricResource(Resource):
    RequestSerializer = QueryTimeSeriesMetricRequestSerializer

    def perform_request(self, validated_request_data):
        paginated_query_set = validated_request_data["paginated_metric_query_set"]

        # 批量获取scope信息
        scope_ids = paginated_query_set.values_list("scope_id", flat=True)
        scopes = models.TimeSeriesScope.objects.filter(
            id__in=scope_ids, group_id=validated_request_data["group_id"]
        ).values("id", "scope_name")
        ...
        for metric in paginated_query_set:
            ...
            metric_dict["scope"] = scope_map.get(metric.scope_id, {"id": metric.scope_id, "name": ""})

            results.append(metric_dict)

        return {"metrics": results, "total": validated_request_data["total"]}
